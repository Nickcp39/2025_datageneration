...
class GaussianDiffusion(nn.Module):
    def __init__(self,
        ...
        # ====== 线性 beta schedule（恢复原始 DDPM 设置） ======
        betas = torch.linspace(beta_1, beta_T, timesteps, dtype=torch.float32, device=device)

        alphas = 1.0 - betas
        alphas_cumprod = torch.cumprod(alphas, dim=0)
        alphas_cumprod_prev = torch.cat(
            [torch.tensor([1.0], dtype=torch.float32, device=device), alphas_cumprod[:-1]], dim=0
        )

        # register_buffer 必须是 torch.Tensor
